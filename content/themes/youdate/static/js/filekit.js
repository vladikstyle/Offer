(function( $ ) {
    jQuery.fn.yiiUploadKit = function(options) {
        var $input = this;
        var $container = $input.closest('.upload-kit');
        var $emptyInput = $container.find('.empty-value');
        var $files = $container.find('.files');
        if (!$files.length) {
            $files = $('<ul>', {"class":"files"}).insertBefore($input);
        }
        ...
        var methods = {
            init: function() {
                ...
                if (options.uploadButtonContent) {
                    $input.after(options.uploadButtonContent);
                } else {
                    $input
                        .after($('<span class="fe fe-plus-circle add"></span>'))
                        .after($('<span class="fe fe-arrow-down-circle drag"></span>'))
                        .after($('<span/>', {"data-toggle":"popover", "class":"fe fe-alert-circly error-popover"}))
                        .after(
                            '<div class="progress">'+
                ...
                var $fileupload = $input.fileupload({
                    ...
                    start: function (e, data) {
                        ...
                        $container.find('.upload-kit-input')
                            .removeClass('error')
                            .addClass('in-progress btn-loading');
                        $input.trigger('start');
                    ...
                    always: function (e, data) {
                        $container.find('.upload-kit-input').removeClass('in-progress btn-loading');
            ...
            showError: function(error) {
                var errorSummary = $container.parents('form').find('.error-summary');
                var list = errorSummary.find('ul')
                list.html('');
                if (typeof error === 'object') {
                    ...
                    errorSummary.find('ul').html($('<li/>').text(error));
                }
                errorSummary.show();
            ...
            createItem: function(file) {
                ...
                var item = $('<li>', {"class": "upload-kit-item done", value: index})
                    ...
                    .append($('<span/>', {"class": "fa fa-trash remove", "data-url": file.delete_url}));
